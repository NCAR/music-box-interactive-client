...
test('Test Reaction Stoichiometry Consistency: Reactants', async () => {
  ...
  arrhenius.data.reactants = [{ name: 'HO2', qty: 1 }, { name: 'HO2', qty: 1}];
  arrhenius.data.products = [{ name: 'H2O2', yield: 1}, { name: 'O2', yield: 1}];
  ...
  arrhenius2.data.products = [{ name: 'H2O2', yield: 1}, { name: 'O2', yield: 1}];
...
test('Test Reaction Stoichiometry Consistency: Products', async () => {
  ...
  arrhenius.data.products = [{ name: 'B', yield: 1}, { name: 'B', yield: 1}];
  await store.dispatch(addReaction(arrhenius));
  ...
  expect(first_reaction.products.B.yield).toEqual(second_reaction.products.B.yield);
});