...
const RemoveReactant = (props) => {
  ...
  const handleRemoveReactant = () => {
    props.removeReactant({ reactionId: props.reactionId, reactantId: props.reactantId });
  };
  ...
  return (
    <div>
      <button type="button"
        ...
        <span className="oi oi-x" toggle="tooltip" aria-hidden="true" title="Remove reactant"></span>
      </button>
    </div>
  );
};
