...
const UploadEvolvingConditions = props => {
  ...
  const convertTextToTable = text => {
    ...
    return [
      ...
      ...rows.map(row => row.split(",").map(val => parseFloat(val.replace(/^"(.*)"$/, '$1'))))
    ]
  }
  ...
  const validateTable = table => {
    ...
    for (let condition of table[0].slice(1)) {
      if (!props.possibleConditions.map(c => c.tableName).includes(condition)) {
        return {
          valid: false,
...
export default connect(mapStateToProps, { loadEvolvingConditionsTable })(UploadEvolvingConditions)
